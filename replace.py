...
for filename in ['frontend/admin.html', 'frontend/receiver.html', 'frontend/script.js']:
    with open(filename, 'r') as f:
        content = f.read()

    # Find the receiptHTML block
    pattern = r'const receiptHTML = `.*?`;'
    
    if filename == 'frontend/admin.html':
        new_template = r"""const receiptHTML = `
            <html>
            <head>
        ...
        `;"""
    elif filename == 'frontend/receiver.html':
        new_template = r"""const receiptHTML = `
            <html>
            <head>
                <title>Shipment Receipt - ${trackingNo}</title>
                <style>
                    body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; padding: 40px; color: #000; max-width: 800px; margin: 0 auto; font-size: 13px; line-height: 1.4; }
                    .header { display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 30px; }
                    .logo-title { display: flex; align-items: center; gap: 15px; }
            ...
            <body>
                ...
                <div class="header">
                    ...
                    <div class="header-date">${new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</div>
                    <div class="header-tracking">${trackingNo}</div>
                </div>
                ...
                <div class="section">
                    ...
                    <div class="grid-2">
                        <div>
                            <div class="col-title">Ship From</div>
                            <div class="info-text upper">PENDING</div>
                        </div>
                        <div>
                            <div class="col-title">Ship To</div>
                            <div class="info-text upper">PENDING</div>
                ...
                <div class="section">
                    <div class="section-title">What</div>
                    <div class="divider"></div>
                    <div class="info-text">Total Shipment Weight: PENDING</div>
                    
                    <div class="parcel-title">Parcel 1 - ${trackingNo}</div>
                    <div class="grid-4">
                        <div>
                            <div class="grid-label">Weight</div>
                            <div class="grid-val">PENDING</div>
                        </div>
                        <div>
                            <div class="grid-label">Dimensions</div>
                            <div class="grid-val">PENDING</div>
                    ...
                    <div class="section-title" style="margin-top:20px;">Service Details - PENDING</div>
                    <div class="yellow-divider"></div>
                    <div class="info-text" style="font-weight: 600;">Estimated Delivery: PENDING</div>
                </div>
                ...
                <div class="section">
                    ...
                    <div class="grid-2">
                        <div>
                            <div class="col-title">Shipping Fees</div>
                            <table class="table no-border">
                                <tr><td>Service</td><td class="right">PENDING</td></tr>
                                <tr class="bold-top"><td>Transportation Charges</td><td class="right"></td></tr>
                            </table>
                        </div>
                        <div>
                            <div class="col-title">Subtotals</div>
                            <table class="table no-border">
                                <tr><td>Shipping Fees</td><td class="right">PENDING</td></tr>
                                <tr class="bold-top"><td>Combined Charges</td><td class="right">PENDING</td></tr>
                            </table>
                        </div>
                    </div>
                    <div class="legal-text" style="margin-top: 10px; font-size:11px;">The rate excludes VAT. Rate includes a fuel surcharge, but excludes taxes, duties and other charges that may apply to the shipment.</div>
    ...
    # Python re.sub
    new_content = re.sub(r'const receiptHTML = `.*?`;', new_template, content, flags=re.DOTALL)
