...
# 1. Remove downloadAdminReceipt button
content = re.sub(
    r'<button class="btn btn-outline-ups btn-lg rounded-0 fw-bold px-4" onclick="downloadAdminReceipt\(\)">[\s\S]*?</button>',
    '',
    content
)

# 2. Replace downloadAdminReceipt function with downloadAdminReceiptFromDashboard
new_func = r"""function downloadAdminReceiptFromDashboard(trackingNumber) {
...
content = re.sub(
    r'function downloadAdminReceipt\(\) \{[\s\S]*?receiptWindow\.document\.close\(\);\n    \}',
    new_func,
    content
)

# 3. Add to dashboard actionBtns desktop
desktop_search = r"if \(s\.payment_status === 'pending_verification'\) \{"
desktop_replace = r"""if (s.payment_status === 'paid') {
                actionBtns = `<button class="btn btn-sm btn-success rounded-0 px-2 ms-1" onclick="downloadAdminReceiptFromDashboard('${s.tracking_number}')">Receipt</button>` + actionBtns;
            }

            if (s.payment_status === 'pending_verification') {"""
content = content.replace(desktop_search, desktop_replace)

# 4. Add to dashboard actionBtns mobile
mobile_search = r"\$\{s\.payment_status === 'pending_verification' \? `<button class=\"btn btn-warning\" style=\"min-height: 48px; border-radius: 24px; font-weight: bold;\" onclick=\"approvePayment\('\$\{s\.id\}'\)\"><span class=\"material-symbols-rounded align-middle\">receipt_long</span></button>` : ''\}"
mobile_replace = r"""${s.payment_status === 'pending_verification' ? `<button class="btn btn-warning" style="min-height: 48px; border-radius: 24px; font-weight: bold;" onclick="approvePayment('${s.id}')"><span class="material-symbols-rounded align-middle">receipt_long</span></button>` : ''}
...
with open('frontend/admin.html', 'w') as f:
    f.write(content)
