...
(async () => {
    ...
    const { data: shipment } = await supabase.from('shipments').insert([{
        sender_id: sender.id, tracking_number: 'FIX2-' + Date.now(),
        status: 'Pending', is_draft: true, payment_status: 'unpaid', shipping_fee: 100
    }]).select().single();
    const token = 'fix2-token-' + Date.now();
    ...
    async function runTest(withAuth) {
        ...
        if (withAuth) {
            // Inject a dummy authenticated session into localStorage exactly as Supabase would
        ...
        let queryResultCount = 'UNKNOWN';
        let supabaseError = 'NONE';
        page.on('console', msg => {
            const t = msg.text();
            if (t.startsWith('returned row count =')) queryResultCount = t.split('=')[1].trim();
            if (t.startsWith('Supabase error =')) supabaseError = t.split('=')[1].trim();
        });
        ...
        const result = await page.evaluate(() => {
            const tabs = document.getElementById('paymentTabs') ? document.getElementById('paymentTabs').innerText : '';
            const content = document.getElementById('paymentTabsContent') ? document.getElementById('paymentTabsContent').innerHTML : '';
            return {
        ...
        await browser.close();
        return { result, queryResultCount, supabaseError };
    }
